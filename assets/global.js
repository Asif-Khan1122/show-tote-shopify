...
class VariantSelects extends HTMLElement {
  ...
  getAllSelectedOptions() {
    const options = [];
    this.querySelectorAll("fieldset, .product-form__input--dropdown").forEach(
      (group) => {
        const checked =
          group.querySelector("input:checked") ||
          group.querySelector("select option[selected]");
        if (checked) {
          options.push({
            name: checked.dataset.optionName || "",
            value: checked.value,
          });
        }
      },
...
class BulkAdd extends HTMLElement {
  ...
  getCartQuantityForLine(id) {
    ...
    return (
      parseInt(
        input?.dataset.cartQuantity || input?.getAttribute("value") || "0",
        10,
      ) || 0
    );
  }
  ...
  startCartLinesUpdate(items) {
    ...
    const linesByAction = Object.entries(items).reduce(
      (groups, [variantId, quantity]) => {
        const nextQuantity = parseInt(quantity, 10);
        const currentQuantity = this.getCartQuantityForLine(variantId);

        if (Number.isNaN(nextQuantity) || currentQuantity === nextQuantity)
          return groups;
        ...
        if (action === "add") {
          ...
          const lineKey = this.querySelector(
            `[data-quantity-variant-id="${variantId}"]`,
          )?.dataset.quantityLineKey;
    ...
    return {
      resolve: (parsedState) => {
        const payload = {
          cart: CartLinesUpdateEvent.createCartFromAjaxResponse(parsedState),
        };
