...
class CartItems extends window.StandardEvents.createViewEventElement(
  HTMLElement,
) {
  constructor() {
    super();
  ...
  static fetchCartData() {
    if (!CartItems.pendingCartDataPromise) {
      const pendingCartDataPromise = fetch(`${routes.cart_url}.json`)
        ...
        .catch(() => null)
        .finally(() => {
          if (CartItems.pendingCartDataPromise === pendingCartDataPromise)
            CartItems.pendingCartDataPromise = null;
        });
  ...
  updateQuantity(line, quantity, event, name, variantId) {
    ...
    const quantityInput =
      this.querySelector(`#Quantity-${line}`) ||
      this.querySelector(`#Drawer-quantity-${line}`);
  ...
  customElements.define(
    ...
    class CartNote extends HTMLElement {
        ...
        this.addEventListener(
          ...
          debounce((event) => {
            ...
            fetch(`${routes.cart_update_url}`, {
              ...
              .then((cart) => {
                if (!cart || cart.errors) {
                  throw Object.assign(new Error(cart?.errors), {
                    code: "INVALID",
                  });
                }
                ...
                if (noteDeferred) {
                  ...
                  if (CartNoteUpdateEvent) {
                    noteDeferred.resolve({
                      cart: CartNoteUpdateEvent.createCartFromAjaxResponse(
                        cart,
                      ),
                    });
                  }
      ...
      dispatchNoteUpdateEvent(newNote) {
        ...
        const context =
          this.closest("dialog") || this.closest("cart-drawer")
            ? "dialog"
            : "cart";
